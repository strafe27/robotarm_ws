...
def callback(data):
    ...
    if accuracy >= 95:
        ...
        # Extract joint angles for publishing, removing the first and last values
        joint_angles = joint_data_df.iloc[0, 1:-4].values.tolist()
    
        # Create a JointState message
        joint_state_msg = JointState()
        joint_state_msg.header.stamp = rospy.Time.now()
        joint_state_msg.name = [f'joint_{i+2}' for i in range(len(joint_angles))]
        joint_state_msg.position = joint_angles
        
        # Publish the message
        joint_pub.publish(joint_state_msg)
        print(f"Accuracy is {accuracy:.2f}")
        print(f"position: {joint_angles}")
    else:
        print(f"Accuracy is {accuracy:.2f}")
...
def main():
    ...
    my_chain = Chain.from_urdf_file("../urdf/robotarm.urdf", active_links_mask=[False, True, True, True, True, True, False])
